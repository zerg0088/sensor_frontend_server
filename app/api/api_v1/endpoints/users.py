...
@router.get("/name/{user_name}", response_model=UserRead, response_model_exclude={"password"})
def read_user_by_name(user_name: str, db: Session = Depends(get_db)) -> Any:
    user = crud_user.get_by_email(db=db, email=user_name)
    print(user)
    if not user:
        return None
    return user
